...
func (p *Parser) writeData(ef *excelize.File, tagMap map[string]Tag, rv reflect.Value) error {
	for i := 0; i < rv.Len(); i++ {
...
func (p *Parser) writeHead(ef *excelize.File, tagMap map[string]Tag, sliceElemType reflect.Type) error {
	heads := make([]string, 0)
